onmessage = function(e) {
  ...
  function printCardWithDelete(producJson,recallAFSCAGtin, recallDGCCRFGtin) {
    try {
      var html = `<div class="product_card container">
   
      <div class="product_card_netcontent label productFull" data='${producJson['gtin']}'>
   ...
   <div class="product_card_country productFull" data='${producJson['gtin']}'>
     ${producJson['countryOfOrigin'] ? "<img class='imageCountry' src='img/country/" + producJson['countryOfOrigin'][0]['countryCode'] + ".svg'>" : ""}
   </div>
   


     <div class="product_card_image productFull" style="background-color: white;background-image: url(${producJson['s:image'] ? producJson['s:image']['s:url']['@id'] : 'img/noPhoto.png'})" data='${producJson['gtin']}'>
     </div>
     <div class="product_card_name">
       ...
       <span class="consultGtin" productFull" data='${producJson['gtin']}'>${producJson['gtin']}<span>
       <h4 class="productFull" data='${producJson['gtin']}'>${producJson['s:name'][0]['@value'].length > "bereidingen van de soort 'Müsli', op basis van niet-".length ? producJson['s:name'][0]['@value'].substring(0, "bereidingen van de soort 'Müsli', op basis van niet-".length) + " ..." : producJson['s:name'][0]['@value']}</h4>`;
     ...
     html += ` <img class="product_card_buttonDel removeHistory" src='img/trash.svg' width="20px" data='${producJson['gtin']}' >
