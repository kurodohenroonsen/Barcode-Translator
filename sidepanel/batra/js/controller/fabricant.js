...
var gpcCode = findGetParameter('GLN');
...
$(document).ready(function () {
	for(var i=0; i<nbrPage-1; i++){
		if((i)==(parseInt(page))){
			document.getElementById("pagination").innerHTML+="<a style='font-size:2em' href='fabricant.html?GLN="+gpcCode+"&page="+i+"&nbrPage="+nbrPage+"'>"+(i+1)+"</a> - ";
	
		}else{
			document.getElementById("pagination").innerHTML+="<a href='fabricant.html?GLN="+gpcCode+"&page="+i+"&nbrPage="+nbrPage+"'>"+(i+1)+"</a> - ";
	...
	if(parseInt(nbrPage)==(parseInt(page))){
		document.getElementById("pagination").innerHTML+="<a style='font-size:2em' href='fabricant.html?GLN="+gpcCode+"&page="+nbrPage+"&nbrPage="+nbrPage+"'>"+nbrPage+"</a>";

	}else{
		document.getElementById("pagination").innerHTML+="<a href='fabricant.html?GLN="+gpcCode+"&page="+nbrPage+"&nbrPage="+nbrPage+"'>"+nbrPage+"</a>";

	}
	document.getElementById("pagination_bottom").innerHTML=document.getElementById("pagination").innerHTML;

	loadJS('js/db/GLN/gln'+gpcCode+'_'+page+'.js', yourCodeToBeCalled, document.body);

	//document.getElementById("manufacturerName").innerHTML = gpcCode;
	
});
function loadProducts(startIndex) {
	var products_container = document.getElementById("products_container");
	products_container.innerHTML = "";
	console.log(glnCode + " : " + glnCode.length);
	for (var i = 0; i < glnCode.length; i++) {
		console.log(glnCode[i]);
		//	for (var i = startIndex; ((i<startIndex+50)||(i < fabricants.length)); i++) {
		printCardProduct(glnCode[i], products_container);
		document.getElementById("manufacturerName").innerHTML = glnCode[i]["manufacturer"]["partyName"];
	}
