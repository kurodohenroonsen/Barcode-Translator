onmessage = function(e) {
    ...
    postMessage(printCardWithDelete(e.data[0], e.data[1], e.data[2], e.data[3], e.data[4], e.data[5]));
    console.log('Envoi du message de retour au script principal');

}

function printCardWithDelete(favoris, recallAFSCAGtin, recallDGCCRFGtin, lastOfferColruyt, lastOfferCarrefour, lastOfferBioplanet) {
    try {
        var gtin = favoris.gtin;
        ...
        find = 'offerPourcentagePlus';
        re = new RegExp(find, 'g');
        lastOfferColruyt = lastOfferColruyt.replace(re, 'offerPourcentagePlusCard');

        find = 'offerPourcentageMoins';
        re = new RegExp(find, 'g');
        lastOfferColruyt = lastOfferColruyt.replace(re, 'offerPourcentageMoinsCard');

        find = '</span>';
        re = new RegExp(find, 'g');
        lastOfferColruyt = lastOfferColruyt.replace(re, '</span><br>');

        find = ' : ';
        re = new RegExp(find, 'g');
        lastOfferColruyt = lastOfferColruyt.replace(re, '');

        find = 'offerPourcentagePlus';
        re = new RegExp(find, 'g');
        lastOfferCarrefour = lastOfferCarrefour.replace(re, 'offerPourcentagePlusCard');

        find = 'offerPourcentageMoins';
        re = new RegExp(find, 'g');
        lastOfferCarrefour = lastOfferCarrefour.replace(re, 'offerPourcentageMoinsCard');

        find = '</span>';
        re = new RegExp(find, 'g');
        lastOfferCarrefour = lastOfferCarrefour.replace(re, '</span><br>');

        find = ' : ';
        re = new RegExp(find, 'g');
        lastOfferCarrefour = lastOfferCarrefour.replace(re, '');

        find = 'offerPourcentagePlus';
        re = new RegExp(find, 'g');
        lastOfferBioplanet = lastOfferBioplanet.replace(re, 'offerPourcentagePlusCard');

        find = 'offerPourcentageMoins';
        re = new RegExp(find, 'g');
        lastOfferBioplanet = lastOfferBioplanet.replace(re, 'offerPourcentageMoinsCard');

        find = '</span>';
        re = new RegExp(find, 'g');
        lastOfferBioplanet = lastOfferBioplanet.replace(re, '</span><br>');

        find = ' : ';
        re = new RegExp(find, 'g');
        lastOfferBioplanet = lastOfferBioplanet.replace(re, '');

        var category = favoris.category;
        var name = favoris.name;

        var label = favoris.label;
        find = 'src="img/score/';
        re = new RegExp(find, 'g');
        ...
        var last_consult = favoris.last_consult;
        var origine = favoris.db_origine;
        find = 'style="height: 33px';
        re = new RegExp(find, 'g');
        origine = origine.replace(re, 'style="height: 16px;"');

        var countryOrigine = favoris.countryOrigine;
        find = '\n\t\t\t\t\t\t\t\t';
        re = new RegExp(find, 'g');
        ...
        var allergens = favoris.allergens;
        find = '<span id=';
        re = new RegExp(find, 'g');
        ...
        <a href="productFull.html?gtin=${gtin}" id="product_card_${gtin}"><table>
            <tr>
                ...
                <td colspan="5"> 
                    <table width="100%">
                        <tr>
                            ...
                            <td class="priceOfferCard">${lastOfferColruyt}</td><td class="priceOfferCard">${lastOfferCarrefour}</td><td class="priceOfferCard">${lastOfferBioplanet}</td>
                        </tr>
                    </table>
                <td>
            </tr>
        ...
        <div style="position: absolute;right: 4px;top: 4px;text-align: right;z-index: 2;display:${recallDisplay}">
